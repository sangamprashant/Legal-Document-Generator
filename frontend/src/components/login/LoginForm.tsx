...
export const LoginForm = ({ status }: { status: "login" | "register" }) => {
    const { login } = useAuth()
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        const userData = { email: trimmedEmail, password: trimmedPassword, ...(status === "register" ? { name: name.trim() } : {}) };
        await login(userData, status);
        setLoading(false);
    };
    ...
    return (
        ...
        <>
            <form onSubmit={handleLogin}>
                ...
                {status === "register" && (
                    <InputField label="Name" type="text" placeholder="Your Name" value={name} onChange={(e) => setName(e.target.value)} />
                )}
